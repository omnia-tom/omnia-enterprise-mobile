...
final class FastVLMService {
  ...
  /// The currently selected model key. Defaults to "qwen2vl2b".
  private(set) var currentModelKey: String = "qwen2vl2b"
  ...
  func setModel(key: String) {
    ...
    if key != currentModelKey {
      ...
      // Force-clear loading state so the new loadModel() can proceed
      isLoading = false
      unloadModel()
      currentModelKey = key
    }
