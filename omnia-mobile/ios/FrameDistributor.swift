...
final class FrameDistributor {
  ...
  /// Called from the frame extraction queue (background). Dispatches to main for display/JS,
  /// passes CVPixelBuffer to recorder and Vision for zero-copy processing.
  func distributeFrame(_ cgImage: CGImage, pixelBuffer: CVPixelBuffer?, timestamp: TimeInterval, width: Int, height: Int) {
    os_unfair_lock_lock(&lock)
    totalFrames += 1
    ...
    // Track FPS (rolling 1-second window) — safe, called from serial frameExtractionQueue
    ...
    // 1. Display views — hop to main thread for CALayer.contents
    if !views.isEmpty {
    ...
    // 2. Recording — pass CVPixelBuffer directly when available (skips CGImage→CVPixelBuffer conversion)
    if let rec = rec {
      if let pb = pixelBuffer {
        rec.writeFrameFromPixelBuffer(pb, timestamp: timestamp)
      } else {
    ...
    // 4. Lightweight metadata event to JS (no pixel data) — must send on main thread
    jsMetadataSkipCount += 1
    if jsMetadataSkipCount >= jsMetadataEveryN {
      jsMetadataSkipCount = 0
      let emitter = eventEmitter
      DispatchQueue.main.async {
        emitter?.sendEvent(withName: "onVideoFrame", body: [
